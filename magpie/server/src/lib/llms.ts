...
run a working card. Include optional referenceAssetIds (1-3 caller-owned ready assets) when
the user attaches reference images; the server validates them and injects them into
generate_asset/batch_generate tool execution even if the model omits or substitutes tool args.
The agent may chain up to 5 tool iterations (e.g. search → generate → add). Available tools:
...
- generate_asset(prompt, transparent?=false, referenceAssetIds?) — generate one brand-on image (active palette + Bloome DNA), persist the PNG to R2, and add it to the library before returning { assetId, status: "ready", mode }. referenceAssetIds is optional, max 3, caller-owned ready assets only. Chargeable; counted in the cost ledger exactly once.
- batch_generate(prompt, count, transparent?=false, cardId?, model?, referenceAssetIds?) — reserve 1-6 pending brand-on image options and return { assetIds, requested, status: "generating" } immediately. Uses the open card/cardId for style inheritance. referenceAssetIds is optional, max 3, caller-owned ready assets only; each pending image shares the same references and materializes through gpt-image-1. Pixels materialize lazily when each asset is polled or fetched; materialization renders one image inline, persists the PNG to R2, writes exactly one imagegen cost row, then starts auto-description.
